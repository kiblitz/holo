...
module Expr = struct
  ...
  let reduce_stack ?next_priority stack =
           ...
           if
             Priority.(
               Comparison_result.equal
                 (compare ~left:priority ~right:next_priority)
                 Reduce)
    ...
    | Expr _ :: _ ->
      Or_error.error_s
        [%message
          "Invariant broken: Consecutive expressions"
            (stack : State.Stack_component.t list)]
  ;;
      ...
      | current_stack ->
        (match%bind.Or_error reduce_stack current_stack with
         | None ->
           Or_error.error_s
             [%message
               "Failed expression reduction" (stack : State.Stack_component.t list)]
    ...
    let rec loop { State.stack; unconsumed_tokens } ~(context : State.Context.t) =
      ...
      | token :: rest_of_tokens ->
        ...
        (match (Source_position.With_section.value token : Token.t) with
         ...
         | Symbol Comma ->
           loop { State.stack = Comma :: stack; unconsumed_tokens } ~context
